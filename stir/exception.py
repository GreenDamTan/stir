...
class StirError(Exception):
    ...
    def __init__(self, details):
        if not type(details) == dict:
            raise Exception('StirError : details must be a dictionary.')
        for key in details:
            if type(key) not in STRING_SET:
                raise Exception('StirError : details key must be strings.')
        if 'message' not in details:
            raise Exception('StirError : details must have message field.')
...
class TimeoutError(StirError):
    def __init__(self, details):
        if type(details) in STRING_SET:
            details = {
                'message': details
            }
        StirError.__init__(self, details)

class RunError(StirError):
...
class WorkspaceError(StirError):
    def __init__(self, details):
        if type(details) in STRING_SET:
            details = {
                'message': details
...
class TestRunnerError(StirError):
    def __init__(self, details):
        if type(details) in STRING_SET:
            details = {
                'message': details
